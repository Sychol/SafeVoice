		...
		if ("/auth/social".equals(path)) {
			if ("google".equals(provider)) {
				...
				String authUrl = new GoogleAuthorizationCodeRequestUrl(googleClientId, googleRedirectUri,
						Collections.singletonList("profile email")).setAccessType("offline").build();
			...
			if ("google".equals(provider)) {
				...
				try {
					...
					// TODO: 로그인 처리 로직 (DB 조회/생성, 세션 설정 등)
					// 예: User user = userService.findOrCreateByEmail(email, name);
					// req.getSession().setAttribute("user", user);

					// 4) 로그인 성공 후 리다이렉트
					resp.sendRedirect(req.getContextPath() + "/GoLogin.do");
					return;
